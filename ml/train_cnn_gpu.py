...
CNN_DIR = Path(__file__).resolve().parent / "cnn"
...
def main():
    ...
    print("=" * 75)
    print("MachineSense ML: GPU-Accelerated Mel-Spectrogram 2D CNN (LOMO Cross-Validation)")
    print("=" * 75)

    # 1. Enforce CUDA GPU Check & Startup Diagnostics
    cuda_available = torch.cuda.is_available()
    print(f" -> torch.cuda.is_available():   {cuda_available}")

    if not cuda_available:
        print("\nFATAL ERROR: CUDA is NOT available!")
        print("This experiment requires CUDA GPU acceleration. Aborting execution.")
        sys.exit(1)
    ...
    sample_model = MelSpectrogramCNN().to(device)
    ...
    # Pre-cache full dataset Mel Spectrogram tensors into RAM using parallel processing
    ...
    # 3. Leave-One-Machine-Out Cross-Validation
    for fold_idx, m_id in enumerate(machine_ids, start=1):
        ...
        for epoch in range(1, epochs_per_fold + 1):
            ...
            val_f1 = f1_score(y_v_true, y_v_pred, zero_division=0)

            print(f"Epoch {epoch:2d}/{epochs_per_fold} | Train Loss: {train_loss:.4f} | Val Loss: {val_loss:.4f} | Val F1: {val_f1:.4f}")
            ...
            if val_loss < best_val_loss:
                ...
                if no_improve_epochs >= patience:
                    stopped_epoch = epoch
                    print(f" -> Early stopping triggered at Epoch {epoch} (Best Val Loss: {best_val_loss:.4f}, Val F1: {best_val_f1:.4f})")
                    break
        ...
        # Save fold checkpoint under ml/cnn/
        fold_ckpt_path = CNN_DIR / f"cnn_mel_spectrogram_{m_id}.pth"
        torch.save(best_model_weights, fold_ckpt_path)

        if m_id == machine_ids[0]:
            torch.save(best_model_weights, CNN_DIR / "cnn_mel_spectrogram_best.pth")
        ...
        print(f" -> Held-Out [{m_id}] Results: Accuracy={acc*100:.2f}%, Precision={prec*100:.2f}%, Abnormal Recall={rec*100:.2f}%, F1={f1:.4f}, ROC-AUC={auc:.4f}")

    total_time = round(time.time() - start_time, 2)

    # 4. Overall Aggregated LOMO Metrics
    ...
    # 5. Save Artifacts under ml/cnn/ and mirror to ml/evaluation/cnn/
    res_df = pd.DataFrame(fold_results)
    res_df_all = pd.concat([res_df, pd.DataFrame([overall_row])], ignore_index=True)

    csv_path_cnn = CNN_DIR / "cnn_lomo_results.csv"
    csv_path_eval = EVAL_DIR / "cnn_lomo_results.csv"
    res_df_all.to_csv(csv_path_cnn, index=False)
    res_df_all.to_csv(csv_path_eval, index=False)
    print(f"\n -> Saved CNN LOMO results CSV: {csv_path_cnn}")

    png_path_cnn = CNN_DIR / "cnn_lomo_confusion_matrices.png"
    png_path_eval = EVAL_DIR / "cnn_lomo_confusion_matrices.png"
    plot_cnn_lomo_confusion_matrices(fold_cms, png_path_cnn)
    plot_cnn_lomo_confusion_matrices(fold_cms, png_path_eval)
    print(f" -> Saved CNN confusion matrix plot: {png_path_cnn}")

    summary_txt_cnn = CNN_DIR / "cnn_summary_report.txt"
    summary_txt_eval = EVAL_DIR / "cnn_summary_report.txt"
    report_content = f"""=== GPU-ACCELERATED MEL-SPECTROGRAM CNN LOMO EVALUATION SUMMARY ===
CUDA Available: {cuda_available}
GPU Device Name: {gpu_device_name}
Selected PyTorch Device: {device}
Total Parameters: {param_count:,}
Total Training Execution Time: {total_time}s
Early Stopping: Active (Patience=5 epochs)

Held-Out Fold Results:
"""
    for r in fold_results:
        report_content += f"Asset [{r['held_out_machine']}]: Accuracy={r['accuracy']:.4f}, Precision={r['precision']:.4f}, Abnormal Recall={r['abnormal_recall']:.4f}, F1={r['f1_score']:.4f}, ROC-AUC={r['roc_auc']:.4f}\n"

    report_content += f"""
Overall Aggregated Performance:
Accuracy: {overall_acc:.4f}
Precision: {overall_prec:.4f}
Abnormal Recall: {overall_rec:.4f}
F1-Score: {overall_f1:.4f}
ROC-AUC: {overall_auc:.4f}
"""

    with open(summary_txt_cnn, "w") as f:
        f.write(report_content)
    with open(summary_txt_eval, "w") as f:
        f.write(report_content)

    # 6. Print Executive Summary Report
    print("\n" + "=" * 75)
    print("MEL-SPECTROGRAM 2D CNN LOMO CROSS-VALIDATION SUMMARY REPORT")
    print("=" * 75)
    print(f"CUDA Available:              {cuda_available}")
    print(f"GPU Hardware:                {gpu_device_name}")
    print(f"PyTorch Device:              {device}")
    print(f"CNN Architecture:            4-Block 2D ConvNet (BatchNorm, LeakyReLU)")
    print(f"Number of Parameters:        {param_count:,}")
    print(f"Training Time:               {total_time} seconds")
    print(f"Early Stopping Used:         Yes (Patience=5 epochs on validation loss)")
    print(f"Model Checkpoint Path:       {CNN_DIR / 'cnn_mel_spectrogram_best.pth'}")
    ...
    print("\n" + "=" * 75)
    print("GPU Mel-Spectrogram CNN Experiment Successfully Completed!")
    print("=" * 75)
